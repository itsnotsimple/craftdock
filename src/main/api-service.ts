  ...
  return new Promise((resolve, reject) => {
    ...
    const handleRequest = (currentUrl: string) => {
      ...
      const requestOptions = {
        headers: {
          'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) CraftDock/2.3.0 (Minecraft Server Manager)',
          Accept: '*/*',
        },
      };
...
export async function searchModrinthModpacks(query = '', limit = 24): Promise<ModrinthSearchResult[]> {
  try {
    ...
    const res = await fetch(url, {
      headers: {
        'User-Agent': 'CraftDock/2.3.0 (contact: github.com/itsnotsimple/craftdock)',
      },
    });
  ...
  try {
    ...
    if (!res.ok) {
      const fallbackRes = await fetch(`https://api.modrinth.com/v2/project/${projectIdOrSlug}/version`, {
        headers: { 'User-Agent': 'CraftDock/2.3.0 (contact: github.com/itsnotsimple/craftdock)' },
      });
